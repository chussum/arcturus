...
```bash
bun run server:up                            # 프로덕션 원클릭: 설치→기동 (여러 번 실행해도 안전). 첫 기동 시 api-swap+ingress+web-swap 순서로 기동. .env.secrets 없고 키 미주입이면 시크릿 설정 게이트(TTY=1.자동생성/2.직접입력/3.안내 메뉴, 비TTY=안내 후 exit 1)
...
bun run secrets:init                         # (macOS, 멱등) 평문 키 파일 → dotenvx 암호화 .env.secrets + Keychain 이전. 키·파일 둘 다 없으면 신규 생성. --input 플래그면 fresh 값을 /dev/tty로 직접 입력(빈칸=자동생성)
...
- **앱 env는 저장 시 AES-256-GCM 암호화** — `apps.env` 컬럼은 `enc:v1:<iv>:<ct||tag>` 포맷(접두사 없으면 레거시 평문). 키는 `ARCTURUS_ENV_KEY` 또는 `data/env-key`(자동 생성, 0600 — fallback이며 부팅 시 WARN 로그). 읽기/쓰기는 `EnvCryptoService`(`infrastructure/crypto/`)를 거쳐야 하며 `parseEnvColumn`은 복호화된 문자열에만 적용. 기존 평문 행은 부팅 시 `EnvEncryptionMigrator`가 자동 재암호화. **키 분실/변경 시 기존 env 복호화 불가** → `decrypt`가 throw하므로 배포가 조용히 실패하지 않음. `data/`는 0700, DB 파일 0600으로 부팅 시 강제.
  - **deploy 시 `.env` 자동 시드(컨테이너, 최초 생성 시만)**: CLI `deploy`가 디렉터리에 Dockerfile + `.env`(또는 `--env-file`)가 있으면 파싱해 멀티파트 `env` 필드(JSON)로 전송(`apps/cli/src/lib/env-file.ts` 파서 + `deploy.ts resolveDeployEnv`). **env 값 정규화는 `env-file.ts`의 `normalizeEnvValue`가 입력 경계(파일 파서·`env --set`)에서 단 한 번 수행** — 값 앞뒤 trim + 공백앞-`#` 인라인 주석 제거(`TOKEN=val # 주석`→`val`), 단 `#fff`·`ab#cd`·URL fragment처럼 공백 없는 `#`는 보존하고 따옴표(`"..."`/`'...'`)로 감싼 값은 안쪽을 그대로 둠(의도적 ` #`·공백 보존용). **API·web은 재정규화 금지** — `normalizeEnvValue`는 따옴표 보존 때문에 비멱등이라 두 번 돌리면 따옴표로 지킨 값을 다시 깎음(API는 이미 파싱된 Record를 받아 그대로 저장, 대시보드 값 칸은 trim만). 컨트롤러가 JSON 파싱(`deployments.envInvalidJson`) 후 `DeploymentsService.create` → `findOrCreateApp`이 **신규 생성 분기에서만** `assertValidEnv`+`EnvCryptoService.encrypt`해 `CreateAppData.env`로 저장(`encryptInitialEnv`). 기존 앱 재배포는 env 무시(대시보드/`env --set`로 넣은 값 보존). 크로스계정 배포는 항상 기존 앱이라 env가 무시되며, CLI는 이 경우 `.env`를 아예 안 읽음(오해 소지 로그 방지). CLI `pack.ts`는 모든 `.env*`(`^\.env(\..*)?$`)를 zip에서 **항상 제외** → 빌드 이미지 레이어에 평문 시크릿 안 남김. 옵션 `--env-file <path>`/`--no-env-file`.
...
- **프로덕션(`NODE_ENV=production`)은 시크릿 fail-closed** — `AppConfig.isProduction`이 true면 `ARCTURUS_JWT_SECRET`(`session.service.ts resolveSecret`)·`ARCTURUS_ENV_KEY`(`env-crypto.service.ts resolveKeyMaterial`)·`ARCTURUS_ADMIN_PASSWORD`(`admin.seeder.ts`) 미설정 시 부팅을 throw로 막음(평문 키 파일 폴백/생성 비번 로그 출력 금지). dev/로컬은 기존 자동 생성 폴백 유지. pm2 `logs/`는 `server-up.sh`가 `chmod 700`. **어드민 비번 최초 시드는 대화형 입력 지원**: admin 없음 + `ARCTURUS_ADMIN_PASSWORD` 미설정 시 `admin.seeder.ts`가 stdin이 TTY면 비번을 대화형(마스킹·재확인·최소 8자)으로 입력받아 시드(prod에서도 throw 대신 프롬프트). TTY 아니면 기존 분기(prod throw / dev 생성+로그). pm2는 TTY가 없으므로 `server-up.sh`가 최초 1회(시드 필요 + TTY + 미설정) 셸에서 프롬프트해 `export` → `api-swap.sh`가 tmp pm2 설정 **env 블록**에 주입(부모 셸 env는 autorestart에 안 남지만 비번은 최초 시드에만 필요). 값이 pm2 dump에 남을 수 있음(저민감·변경 가능). **시드 필요 판별은 `needs_admin_seed()`** — DB 파일 부재 **또는** DB는 있으나 유저 0명(시더 조건 `users.list().length===0`과 동일). 부팅 마이그레이션이 admin 시드보다 먼저 `arcturus.db`(스키마만)를 만들기 때문에 "DB 파일 부재"만으로 판별하면, 실패한 첫 부팅이 남긴 빈 DB에서 게이트가 영구 스킵 → 비번 미주입 → 시더 재throw → **크래시 루프**가 된다(실측). 유저 수는 `sqlite3 -readonly -cmd ".timeout 2000"`로 읽고(`.timeout`은 출력 없는 dot-command — `PRAGMA busy_timeout=`은 값을 출력해 카운트 파싱을 깨뜨림), 읽기 실패/`sqlite3` 부재 시 "시드됨"으로 폴백해 건강한 설치의 오프롬프트 회귀를 막는다.
- **배포 컨테이너 하드닝** — `dockerode.adapter.ts runContainer`의 `HostConfig`에 `CapDrop:['ALL']` + `SecurityOpt:['no-new-privileges']`(CPU/PID 제한과 함께), 그리고 ICC 끈 전용 브리지 `arcturus-apps`(`ensureAppNetwork`가 멱등 생성, `NetworkMode`로 부착)에 배치해 테넌트 간 도달 차단. egress(아웃바운드)는 유지. ReadonlyRootfs는 앱이 디스크 쓰기를 자주 해 깨질 수 있어 의도적으로 미적용.
  ...
  - **고아 컨테이너 boot sweep** — `runContainer`가 생성하는 모든 컨테이너에 라벨 `arcturus.managed=true` + `arcturus.app-id=<appId>`를 박는다(`dockerode.adapter.ts`, `RunContainerOptions.appId`). 부팅 시 `ContainerStateReconciler`가 상태 reconcile 후 `runtime.listManaged()`로 라벨 컨테이너를 훑어 **DB 앱 행이 없는** app-id의 컨테이너를 `removeContainer`로 제거(best-effort, try/catch+WARN). 계정/앱 삭제 purge가 best-effort라 docker 일시 오류로 컨테이너만 남으면 `RestartPolicy: unless-stopped` 탓에 영원히(재부팅에도) 살아남는데, 이 sweep이 다음 부팅에서 회수한다. **라벨 도입 이전(레거시) 컨테이너는 app-id가 null이라 sweep 대상에서 제외** — 다음 재배포 때 라벨이 붙어 관리권에 편입. app-id가 살아있으면 절대 안 지운다.
  - **빌드 타임아웃은 데몬측 빌드를 취소하지 못함(알려진 한계)** — `buildImage`의 15분 타임아웃은 클라이언트 HTTP 스트림만 destroy하고, 빌드는 dockerd 내부에서 계속 돈다(classic/BuildKit 모두 클라 스트림 끊김 ≠ build abort). dockerode/Docker API에 신뢰할 빌드 취소 수단이 없어 메모리 상한으로만 자원을 bound한다(억지 취소는 dangling 이미지/캐시 부작용이 더 큼).
  - **pm2 `treekill: true` 명시** — API/web 블루-그린 임시 ecosystem JSON(`api-swap.sh`·`web-swap.sh`)에 `treekill: true`를 명시한다. API 시그널 체인은 `pm2 → exec bunx → dotenvx run → bun`(`with-secrets.sh`)이라 pm2가 추적하는 최상위만 SIGKILL하면 `bun` 손자가 launchd로 reparent돼 내부 포트 + SQLite WAL 핸들을 쥔 채 고아로 살아남을 수 있다. treekill은 stop 시 전체 프로세스 트리에 시그널을 보내 손자까지 reap하는 backstop(graceful drain은 dotenvx 시그널 포워딩에 의존, treekill은 하드킬 보강). pm2 기본값이 true지만 버전 드리프트 대비 명시.
